...
- **Quote verifier** — deterministic, zero model cost: every quoted line in the persona and research files is checked verbatim against the raw corpus (green = traceable with date + source file; red = not found, for human judgment). Doubles as the ground-truth anchor for evolution scoring. Plus a **quotebook**: the subject's real utterances straight from the corpus, grouped by month, searchable.
- **Weekly review** — one call that digests the past week's chats, journal entries, and prediction outcomes into a short honest reflection with one small suggestion.
- **Hands-free voice mode** — replies auto-spoken, mic auto-opens after speech ends, auto-send on final result. Browser-native, free.
- **One-click cloud deploy** — `render.yaml` blueprint (requires `NUWA_PASSWORD`; persistent disk needs a paid plan).
...
- **Fun pack** — *Guess-their-reply*: a quiz drawn from real exchanges (context shown, their actual reply hidden; guess, reveal, optionally let the persona answer the same prompt — a free, honest fidelity calibration). *Dialogue simulator*: two personas talk to each other automatically (distill yourself and watch "you × them"; same person with two time cutoffs = past-them × present-them). *Relationship Wrapped*: a scrollable year-in-review. *Daily fortune*: one real utterance drawn deterministically per day.
